...
const GITHUB_API_URL_TEMPLATE =
  "https://api.github.com/users/{username}/repos?per_page={perPage}";
const GITHUB_PAGES_SITEMAP_TEMPLATE =
  "https://{username}.com/{repo}/sitemap.xml";
...
async function checkRepositoryForSitemap(
  ...
  const sitemapUrl = GITHUB_PAGES_SITEMAP_TEMPLATE.replace(
    "{username}",
    username
...
async function fetchUserRepositories(username: string): Promise<Repository[]> {
  const apiUrl = GITHUB_API_URL_TEMPLATE.replace(
    "{username}",
    username
  ...
  console.log(
    LOG_FOUND_REPOSITORIES.replace(
      "{count}",
...
async function checkRepositoriesForSitemaps(
  ...
  const sitemapPromises = repositories.map(repo =>
    checkRepositoryForSitemap(repo.name, username)
  );
